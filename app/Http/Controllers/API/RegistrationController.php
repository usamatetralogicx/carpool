         ...
         public function profileupdate(Request $request,$id)
   {
        $rules=array(
            'phone' => 'min:10|numeric',
             'password' => ['min:8','regex:/^(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@_$%^&*-]).{6,}$/'],
    ...
    public function sentPasswordResetLink(Request $request)
    {
        ...
        if($user){
            passwordreset2::where('email', $request->email)->delete();
             $value=rand ( 1000 , 9999 );
    ...
    {
        $user =passwordreset2::where('user_id',$request->user_id)->first();
        if($user)
        {
            $verify =passwordreset2::where('token',$request->token)->first();
            if($verify)
            {
